#functions for our xword generator

#initialize an empty crossword
...
#remove empty spaces from the edges of the matrix
...
# Find nearest manhattan distances
...
run_MCMC <- function(xword, word_locs, n_iter = 100, print_progress = F, draw_progress = F){
  ...
  for(i in 1:n_iter){
    ...
    if(rbinom(1, 1, prob = accept_prob) == 1){
      ...
      if(draw_progress){
        ...
        png(paste0("output/img/mcmc_progress/xword_", i,".png"), family = font_family_for_nums, width = ncol(trimmed_xword_data$xword) * 30 * ptsize_x, 
